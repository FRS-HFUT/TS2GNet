...
class dataLoad(torch.utils.data.Dataset):
    ...
    def __init__(self, data, gt, patch_size, center_pixel=False, flip_augmentation=True, radiation_augmentation=False, mixture_augmentation=True):
        """
        Args:
...
class dataLoad1(torch.utils.data.Dataset):
    """ Generic class for a hyperspectral scene """
    def __init__(self, data, gt, patch_size=16, center_pixel=False, flip_augmentation=True, radiation_augmentation=True, mixture_augmentation=True):
        """
        Args:
            data: 3D hyperspectral image---->(m, n, c)
            gt: 2D array of labels---->(m,n)
